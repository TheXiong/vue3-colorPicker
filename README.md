<h1 align="center">x-color-picker</h1>

<p align="left">
  <strong>组件名称</strong>: <code>x-color-picker</code>
</p>

<p align="left">
  <strong>Github 仓库</strong>: <a href="https://github.com/TheXiong/vue3-colorPicker">https://github.com/TheXiong/vue3-colorPicker</a>
</p>

## 1. 组件概述

`x-color-picker` 是一款基于 Vue 3 + TypeScript 开发的高级颜色选择器组件。它不仅支持标准的单色（Solid）选择，还内置了强大的渐变色（Gradient）编辑功能。组件集成了透明度调节、HSV/RGB/HEX 格式切换、系统预设颜色面板以及智能的“最近使用颜色”记录功能。其设计目标是为开发者提供一个开箱即用、交互流畅且高度可配置的色彩解决方案，适用于各类设计工具、后台管理系统及个性化配置场景。
...
import { ref } from 'vue';
import { XColorPicker } from 'x-color-picker';
import 'x-color-picker/dist/x-color-picker.css'; // 引入样式
