...
.ik-color-picker {
  ...
  &__trigger {
    ...
    border-radius: 3px;
    cursor: pointer;
    background: var(--x-cp-bg-trigger);
    /* Dark mode bg */
    transition: all 0.2s;
    height: 32px;
    ...
    .arrow-icon {
      ...
      border-top: 5px solid var(--x-cp-text-secondary);
      margin-left: 6px;
  ...
  &__clear {
    ...
    position: absolute;
    right: 4px;
    background: var(--x-cp-bg-clear);

    &:hover {
      background: var(--x-cp-bg-clear-hover);
      color: var(--x-cp-text-inverse);
    }
  }
...
.ik-color-picker__panel-container {
  ...
  box-shadow: 0 4px 6px var(--x-cp-shadow-color-base);
  border-radius: 4px;
...
.ik-color-picker__overlay {
  position: fixed;
  inset: 0;
  background-color: var(--x-cp-bg-overlay);
}
...
/* Transition styles */
.x-slide-fade-enter-active,
.x-slide-fade-leave-active {
  transition: all var(--animation-duration, 0.2s) ease;
}

.x-slide-fade-enter-from,
.x-slide-fade-leave-to {
