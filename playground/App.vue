...
import XColorPicker from '../src/index.vue';
...
const customSwatchesColor = ref(
  'linear-gradient(190deg, rgba(255, 0, 183, 1) 0.00%, rgba(0, 91, 255, 1) 49.30%, #000000 100.00%)',
);
...
<template>
  <div class="page-container">
    ...
    <header class="hero">
      <div class="hero-content">
        ...
        <div class="install-cmd">
          <code>npm install x-color-picker</code>
          <button
            class="copy-btn"
            @click="() => navigator?.clipboard.writeText('npm install x-color-picker')"
          >
            Copy
          </button>
        </div>
    ...
    <footer class="footer">
      <p>XColorPicker &copy; {{ new Date().getFullYear() }}</p>
    </footer>
  </div>
</template>
